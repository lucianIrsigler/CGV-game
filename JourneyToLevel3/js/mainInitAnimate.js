...
let deltaTime, intersects, rulesBoardInnerHtml, targetPosition, forwardDirection
let firstPersonLookAt
// END: Animate Function Variables ================
...
let isHitBySpotlight;
let spotLightTargetLastStage;
let spotLightLastStageFollowSpeed;
let ambientLight;
let spotLightTarget;
let spotLight;
let spotLightSpeed;
let sLightHelper;
let oscilateXSpotLight;
//#endregion LIGHTING variables =========

//#region OBJECTS variables ============
let duckGroundCollission;
let boxCollision;
...
//#region VARIABLES BEFORE ANIMATE FUNCTION =========================
...
//#endregion VARIABLES BEFORE ANIMATE FUNCTION =========================

//#region GUI HELPER VARIABLES =========================
let gui
let options
//#endregion GUI HELPER VARIABLES =========================

//#region EVENTS HANDLERS VARIABLES =========================
let mousePosition
let keys
let canJump
let jumpCooldown

//#endregion EVENTS HANDLERS VARIABLES =========================
...
const initScene = () => {
    ...
    isHitBySpotlight = (spotlight, objectPosition) => {
    ...
    ambientLight = new THREE.AmbientLight(0xfffffff, 0.05)
    ...
    spotLight = new THREE.SpotLight(0xffffff, 10000)
    scene.add(spotLight)
    ...
    duckGroundCollission = ({ duck, ground }) => {
    ...
    lastStageGroundWidth = groundWidth * 4
    lastStageGroundDepth = groundDepth * 15
    lastStageGroundHeight = groundHeight * 2

    groundGap = 15
    groundYDisplacement = 5
    groundXDisplacement = groundWidth * 2
    groundZLocationTracker = groundDepth / 2;
    ...
    previousGroundX = 0
    previousGroundY = groundYDisplacement

    createGrounds = (numGrounds = 10) => {
    ...
    canJump = true; // Boolean to track if jumping is allowed
    jumpCooldown = 1000;
    ...
    //ambient sound
    ambientSound = new Audio('../assets/alexander-nakarada-chase(chosic.com).mp3');
...
const animate = (time) => {
    ...
    if (gameStarted) {
        ...
        if (duckModel && followDuck) {
            ...
            targetPosition = duckModel.position.clone().add(followOffset);
            ...
            if (!options.firstPerson) {
                ...
                forwardDirection = new THREE.Vector3(0, 0, -1);  // Duck is facing negative Z (forward)
                ...
                firstPersonLookAt = duckModel.position.clone().add(forwardDirection.multiplyScalar(10));
